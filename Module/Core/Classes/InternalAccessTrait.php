...
trait InternalAccessTrait
{
    ...
    public function getProperty(string $key)
    {
        if (!$this->hasProperty($key)) {
            throw new BetterApiException('The object ' . get_class($this->getExecutionTarget()) . " does not have a property, called: $key");
        }
    ...
    public function callMethod(string $method, array $args = [])
    {
        if (!$this->hasMethod($method)) {
            throw new BetterApiException('The object ' . get_class($this->getExecutionTarget()) . " does not have a method, called: $method");
        }
