...
class TranslationService implements SingletonInterface {
	...
	public function addContext(string $context, string $filename): TranslationService {
		...
		if (stripos($filename, "lll:") === 0) $filename = substr($filename, 0, 5);
	...
	public function getTranslationKey(string $selector): string {
		...
		if ($pr === static::PARSE_RESULT_ALREADY_TRANSKEY) return $this->resolveOverride($selector);
		if ($pr === static::PARSE_RESULT_NOT_TRANSLATABLE) $this->requireContext($context, $selector);
	...
	public function translate(string $selector, array $args = []): string {
		...
		if ($this->tsfe->hasTsfe()) $result = $this->tsfe->getTsfe()->sL($key);
		else $result = $this->getTypoLanguageService()->sl($key);
		if (!is_string($result)) $result = "";
		if (!empty($args)) $result = vsprintf($result, $args);
		return $result;
	}
	...
	public function translateMaybe(string $selector, array $args = []): string {
		...
		if (empty($result) && $this->isTranslatable($selector)) return "";
		return $result;
	}
	...
	public function getTypoLanguageService(): LanguageService {
		if ($this->tsfe->hasTsfe()) return TsfeAdapter::getLanguageService($this->tsfe->getTsfe());
	...
	public function getAllKeysInFile(string $filename): array {
		if ($this->hasContext($filename)) $filename = $this->getContextFile($filename);
		...
		$labels = array_map(function ($v) use ($filename) { return $filename . ":" . $v; }, $labels);
	...
	protected function parseSelector(string &$selector, &$context = NULL): int {
		...
		} else return self::PARSE_RESULT_ALREADY_TRANSKEY;
		...
		if (!$this->hasContext($context)) return static::PARSE_RESULT_NOT_TRANSLATABLE;
	...
	protected function resolveOverride(string $selector): string {
		if (empty($this->overrides)) return $selector;
		
		// Unify path's relative to an extension
		if (stripos($selector, "lll:") !== 0) $selector = "LLL:" . $selector;
		...
		while (isset($this->overrides[$selector])) {
			...
			if ($c++ > 10) throw new TranslationException("More than 10 subsequent overrides are not supported. Maybe a circular override?");
		}
