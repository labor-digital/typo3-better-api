...
class SyncHandler implements SingletonInterface {
	...
	public function sync(Iterator $iterator, array $options) {
		...
		foreach ($iterator as $k => $v) {
			if (!$v instanceof SplFileInfo) continue;
			...
			// Add file to list
			if (empty($langKey)) $fileSets[$basename]["baseFile"] = $v->getPathname();
			else $fileSets[$basename]["langFiles"][$langKey] = $v->getPathname();
		}
	...
	protected function syncFileSet(array $fileSet, array $languages, array $options) {
		...
		if (count($languages) !== count($fileSet["langFiles"])) {
			foreach ($languages as $language => $foo) {
				if (isset($fileSet["langFiles"][$language])) continue;
	 ...
	 */
	protected function loadFile(string $filename, ?string $languageFallback = NULL,
								?string $baseLanguage = NULL, ?string $productName = NULL): TranslationFile {
		...
		foreach (Arrays::getPath($contentList, "0.0.*", []) as $entry) {
			if (!isset($entry["tag"]) || $entry["tag"] !== "body") continue;
			foreach ($entry as $k => $row) {
				...
				else if ($row["tag"] === "trans-unit") {
					foreach ($row as $_k => $child) {
						if (is_string($_k) || !is_array($child)) continue;
