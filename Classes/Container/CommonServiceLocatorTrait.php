...
trait CommonServiceLocatorTrait {
	...
	public function getInstanceOf(string $class, array $args = []) {
		...
		if (empty($this->__container)) $this->__container = TypoContainer::getInstance();
	...
	public function __get($name) {
		...
		if (!empty($this->__instances[$name])) return $this->__instances[$name];
		...
		if (is_object($definition)) {
			...
			if ($definition instanceof Closure) return $this->__instances[$name] = call_user_func($definition, $this->Container);
	...
	private function __initializeTrait() {
		if ($this->__traitInitialized) return;
