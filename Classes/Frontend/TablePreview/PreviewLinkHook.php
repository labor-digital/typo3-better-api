...
class PreviewLinkHook
{
    ...
    public function postProcess(
        ...
        if ($configuration['generator']) {
            /** @var \LaborDigital\Typo3BetterApi\Frontend\TablePreview\PreviewLinkGeneratorInterface $generator */
            $generator = $this->getInstanceOf($configuration['generator']);
        ...
        return $this->getInstanceOf(PreviewHandler::class)->appendUrlValueToLink($link, $tables);
    }
    ...
    protected function internalGenerator(string $uid, array $configuration): string
    {
        ...
        if (is_string($pid) && $this->getInstanceOf(LinkSetRepository::class)->has($pid)) {
            $link = $this->Links()->getLink($pid);
