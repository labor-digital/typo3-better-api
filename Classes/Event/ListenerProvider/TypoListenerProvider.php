...
class TypoListenerProvider extends OrderedListenerProvider {
	...
	public function addListener(callable $listener, int $priority = 0, string $id = NULL, string $type = NULL): string {
		if ($this->registerSpecialEvents($type, $listener))
			return parent::addListener($listener, $priority, $id, $type);
		return md5($type . rand());
	}
	...
	public function addListenerBefore(string $pivotId, callable $listener, string $id = NULL, string $type = NULL): string {
		if ($this->registerSpecialEvents($type, $listener))
			return parent::addListenerBefore($pivotId, $listener, $id, $type);
		return md5($type . rand());
	}
	...
	public function addListenerAfter(string $pivotId, callable $listener, string $id = NULL, string $type = NULL): string {
		if ($this->registerSpecialEvents($type, $listener))
			return parent::addListenerAfter($pivotId, $listener, $id, $type);
		return md5($type . rand());
	}
	 ...
	 * @return bool
	 */
	protected function registerSpecialEvents(string $eventClass, callable $listener): bool {
		if ($this->registerSignalSlotIfRequired($eventClass, $listener)) return FALSE;
		$this->registerCoreHookEventIfRequired($eventClass);
		return TRUE;
	}
	
	/**
	 * Internal helper to call the bind method if the given $eventClass implements the core hook event interface
	 *
	 * @param string $eventClass
	 *
	...
	protected function registerCoreHookEventIfRequired(string $eventClass): void {
		...
		if (isset($this->boundCoreHooks[$eventClass])) return;
		if (!class_exists($eventClass)) return;
		if (!in_array(CoreHookEventInterface::class, class_implements($eventClass))) return;
		...
		if (!in_array(CoreHookEventAdapterInterface::class, class_implements($adapterClass)))
			throw new EventException("The class \"$eventClass\" returned \"$adapterClass\" as it's core hook adapter, but the adapter does not implement the required interface: \"" . CoreHookEventAdapterInterface::class . "\"!");
		if (isset($this->boundCoreHooks[$adapterClass])) return;
		
		// Bind the adapter
		call_user_func([$adapterClass, "prepare"], TypoEventBus::getInstance(), $this->container->get(TypoContext::class), $this->container);
	...
	/**
	 * Internal helper to register the signal in the extbase signal slot dispatcher if we don't know it yet.
	 *
	 ...
	 * @throws \LaborDigital\Typo3BetterApi\Event\EventException
	 */
	protected function registerSignalSlotIfRequired(string $eventClass, callable $listener): bool {
		// Check if a valid "className.signal" event was given
		if (class_exists($eventClass) || stripos($eventClass, ".") === FALSE || count(explode(".", $eventClass)) !== 2)
			return FALSE;
