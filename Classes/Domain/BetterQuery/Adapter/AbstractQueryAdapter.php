...
abstract class AbstractQueryAdapter {
	...
	public function ensureArrayValue($value, string $field): array {
		if ((!is_array($value))) {
			if ((is_string($value) || is_numeric($value))) $value = Arrays::makeFromStringList($value);
			else if ($value instanceof ObjectStorage) $value = $value->toArray();
			else if ($value instanceof LazyObjectStorage) $value = $value->toArray();
			else throw new DbServiceException("Invalid value for \"$field\" given! Only strings, numbers or arrays are allowed!");
		}
		return $value;
	}
}