...
class BetterQuery extends AbstractBetterQuery {
	 ...
	 * @inheritDoc
	 */
	public function __construct(BetterRepository $repository, QueryInterface $query, TypoContext $typoContext, Session $session) {
		parent::__construct(
	...
	public function withIncludeHiddenChildren($settings = TRUE): BetterQuery {
		$clone = clone $this;
		if (empty($settings)) $clone->includeHiddenChildren = NULL;
		else $clone->includeHiddenChildren = $settings;
		return $clone;
	}
	...
	public function withIncludeDeletedChildren($settings = TRUE): BetterQuery {
		$clone = clone $this;
		if (empty($settings)) $clone->includeDeletedChildren = NULL;
		else $clone->includeDeletedChildren = $settings;
		return $clone;
	}
	...
	public function getQueryBuilder(): QueryBuilder {
		return BetterQueryTypo3DbQueryParserAdapter::getConcreteQueryParser()->convertQueryToDoctrineQueryBuilder($this->getQuery());
	}
	 ...
	 *
	 * @param bool $returnAsArray If set to true the method will return the raw database arrays instead of the extBase
	 *                            objects. NOTE: If you set this to true there will be no relation resolving!
	 *
