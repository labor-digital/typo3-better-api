...
use LaborDigital\Typo3BetterApi\Event\Events\ClassOverrideContentFilterEvent;
use LaborDigital\Typo3BetterApi\Event\Events\ClassOverrideStackFilterEvent;
...
class ClassOverrideGenerator {
	 ...
	 * @return mixed
	 */
	public static function __classLoader($class) {
	 ...
	 *
	 * @throws \LaborDigital\Typo3BetterApi\CoreModding\CodeGeneration\ClassOverridesException
	 */
	public static function registerOverride(string $classToOverride, string $classToOverrideWith, bool $overrule = FALSE) {
		if (class_exists($classToOverride, FALSE))
			throw new ClassOverridesException("The class: " . $classToOverride . " can not be overridden, because it is already defined!");
		if (static::hasClassOverride($classToOverride) && !$overrule)
			throw new ClassOverridesException("The class: " . $classToOverride . " is already overridden with: " . static::$overrideDefinitions[$classToOverride] . " and therefore can not be overwritten again!");
	...
	public static function canOverrideClass(string $classToOverride, bool $withOverrule = FALSE): bool {
		if (class_exists($classToOverride, FALSE)) return FALSE;
		if (!isset(static::$overrideDefinitions[$classToOverride])) return TRUE;
		if ($withOverrule) return TRUE;
		return FALSE;
	}
	 ...
	 * @return string
	 */
	protected static function resolveOverrideStack(array $stack): string {
		...
		foreach ($stack as $classToOverride => $classToOverrideWith) {
			...
			if (!static::$fs->hasFile($aliasFilename) || !static::$fs->hasFile($cloneFilename)) {
				...
				$aliasContent = static::getClassAliasContent($classToOverride, $classToOverrideWith, $finalClassName, $copyClassFullName);
				
				// Allow filtering
				$e = new ClassOverrideContentFilterEvent(
					$classToOverride, $copyClassName, $initialClassName, $finalClassName,
					$cloneContent, $aliasContent
				);
				TypoEventBus::getInstance()->dispatch($e);
	 ...
	 * @return string
	 */
	protected static function getClassAliasContent(string $classToOverride, string $classToOverrideWith, string $finalClassName, string $copyClassFullName): string {
		$namespace = Path::classNamespace($classToOverride);
...
";
	...
	protected static function getClassCloneContentOf(string $of, string $copyClassName): string {
		...
		if ($overrideSourceFile === FALSE)
			throw new ClassOverridesException("Could not create a clone of class: " . $of . " because Composer could not resolve it's filename!");
		...
		if (!$nameChanged)
			throw new ClassOverridesException("Failed to rewrite the name of class: " . $className .
				" to: " . $copyClassName . " when creating a copy of file: " . $overrideSourceFile);
		
		// Fix return types
		foreach ($source as $k => $line) {
			if (stripos($line, "@return") === FALSE) continue;
		...
		foreach ($source as $k => $line) {
			...
			$sourceParsed[] = $line;
			if (!$noticeSet && (stripos($line, "<?php") !== FALSE || stripos($line, "<?=") !== FALSE || stripos($line, "<?") !== FALSE)) {
				...
				$sourceParsed[] = " * " . PHP_EOL;
				$sourceParsed[] = " * It was created by the LABOR Typo3BetterApi extension in order to extend core functionality." . PHP_EOL;
