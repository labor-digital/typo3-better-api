...
class DataHandlerActionHandler implements SingletonInterface, DataHandlerActionHandlerInterface {
	 ...
	 */
	public function __construct(TypoContainerInterface $container, EventBusInterface $eventBus,
								DbService $dbService, FlexFormTools $flexFormTools) {
	...
	public function runActionStack(string $stackType, string $tableName, $uid, object $event, array &$row, &$isDirty = FALSE) {
		...
		$givenRow = array_map(function ($v) { return $v; }, $row);
		...
		} else $rowType = BackendUtility::getTCAtypeValue($tableName, $row);
		
		// Backup the original row
		$rawRow = array_map(function ($v) { return $v; }, $row);
		...
		if ($this->hasContextEmptyListCacheFor($stackType, $uid, $tableName, $rowType)) return;
		
		// Unpack all flex form fields
		$unpackedFlexFormFields = [];
		foreach ($row as $k => $v) {
			if (!isset($tcaColumns[$k])) continue;
			if (Arrays::getPath($tcaColumns, [$k, "config", "type"]) !== "flex") continue;
			if (is_array($v)) continue;
			$unpackedFlexFormFields[] = $k;
			$unpacked = empty($v) ? [] : GeneralUtility::xml2array($v);
			if (!is_array($unpacked)) $unpacked = [];
			$row[$k] = $unpacked;
		}
		...
		if (!empty($contextList)) {
			...
			foreach ($contextList as $contextConfig) {
				...
				$this->eventBus->dispatch(($e = new BackendFormActionContextFilterEvent(
					$contextClass, $contextConfig, $stackType
				)));
				...
				if ($contextObject->isValueDirty()) {
					...
					if ($contextConfig["appliesToTable"]) $row = $contextObject->getValue();
					
					// Update a single value
					else $row = Arrays::setPath($row, $contextObject->getPath(), $contextObject->getValue());
		...
		if (!empty($unpackedFlexFormFields)) {
			foreach ($unpackedFlexFormFields as $field)
				if (is_array($row[$field])) $row[$field] = $this->flexFormTools->flexArray2Xml($row[$field]);
		}
		
		// Make sure to only update the changed values
		$rowChanged = $givenRow;
		foreach ($row as $k => $v) if ($v != $rawRow[$k]) $rowChanged[$k] = $v;
		$row = $rowChanged;
	}
	...
	protected function readContextListForStack(string $stackType, $uid, array $row, string $tableName, array $tca, string $type): array {
		...
		$cacheKey = md5($stackType . $uid . $tableName . $type);
		if (isset($this->contextListCache[$cacheKey])) return $this->contextListCache[$cacheKey];
		...
		foreach ($row as $key => $value) {
			...
			if (!isset($tca["columns"][$key])) continue;
			$tcaField = $tca["columns"][$key];
			
			// Skip if there is no config
			if (!is_array($tcaField) || !is_array($tcaField["config"])) continue;
		...
		if (!empty($handlers)) {
			...
			foreach ($handlers as $handler) {
				...
				if (count(array_intersect_assoc($handler["constraints"], $row)) !== count($handler["constraints"])) continue;
	...
	protected function readContextListFromFlexForm(string $configKey, array $value, array $tcaField, string $tableName, string $key, array $row): array {
		...
		foreach (Arrays::flatten($structureArray) as $k => $v) {
			...
			if (!strpos($k, ".dataHandlerActions." . $configKey . ".")) continue;
			...
			array_pop($path);
			array_pop($path);
			if (end($path) !== $configKey) continue;
			array_pop($path);
			...
			foreach ($path as $partId => $part) {
				...
				if (!is_array($pointer)) break;
				
				// Translate special keys
				if (!isset($pointer[$part]))
					if ($part === "sheets") $part = "data";
					else if ($part === "ROOT") $part = "lDEF";
				...
				if (is_array($pointer) && is_array($pointer["el"])) {
					foreach ($pointer["el"] as $k => $el) {
						...
						foreach ($el as $_k => $item) {
							// Ignore broken structures
							if (!is_array($item) || !isset($item["el"])) continue;
							$valuePath[] = $_k;
							$valuePath[] = "el";
	 ...
	 * @return array
	 */
	protected function addContextsForStack(string $configKey, array $config, array $contexts,
										   array $path = [], string $fieldName = "", $value = NULL): array {
		// Ignore if the config does not have the required config key
		if (!is_array($config[$configKey])) return $contexts;
