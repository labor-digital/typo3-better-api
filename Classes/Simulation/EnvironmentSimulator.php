...
class EnvironmentSimulator implements SingletonInterface
{
     ...
     *                              the
     *                              $language was not found for this site. If true is given, the default language will
     *                              be used
    ...
    public function runWithEnvironment(array $options, callable $handler)
    {
        ...
        if ($this->isInSimulation && $options['ignoreInSimulations']) {
            $ignoreSimulation = true;
        } elseif ($this->isInSimulation && $this->childSimulationsIgnored) {
            $ignoreSimulation = true;
        }
        
        // Ignore, because frontend exists
        if (! $ignoreSimulation && $options['ignoreIfFrontendExists'] && $this->Tsfe->hasTsfe()) {
            $ignoreSimulation = true;
        }
        ...
        if (! $ignoreSimulation) {
            ...
            if ($options['bootTsfe'] && ! $this->Tsfe->hasTsfe()) {
                $doSimulation = true;
            }
            ...
            if (! $doSimulation && ($options['language'] !== null)) {
                ...
                if ($languageObject->getLanguageId() !== $this->TypoContext->getLanguageAspect()
            ...
            if (! $doSimulation
                && $options['includeHiddenPages'] !== $this->TypoContext->getVisibilityAspect()->includeHiddenPages()) {
                $doSimulation = true;
            }
            if (! $doSimulation
                && $options['includeHiddenContent'] !== $this->TypoContext->getVisibilityAspect()
                                                                          ->includeHiddenContent()) {
                $doSimulation = true;
            }
            if (! $doSimulation
                && $options['includeDeletedRecords'] !== $this->TypoContext->getVisibilityAspect()
                                                                           ->includeDeletedRecords()) {
                $doSimulation = true;
            }
        }
        ...
        try {
            if ($doSimulation) {
                ...
                try {
                    ...
                    // Visibility
                    // =============================
                    if ($options['includeHiddenPages'] || $options['includeHiddenContent']
                    ...
                    if ($options['language'] !== null) {
                        ...
                        $backup->language        = $this->TypoContext->getLanguageAspect()
                                                                     ->getCurrentFrontendLanguage();
                        if (isset($GLOBALS['LANG'])) {
                            $backup->langService = $GLOBALS['LANG'];
                        }
                        $backup->langAspect = $this->TypoContext->getRootContext()->getAspect('language');
                        if (isset($GLOBALS['TYPO3_REQUEST'])) {
                            $backup->typoRequest = $GLOBALS['TYPO3_REQUEST'];
                        }
                    ...
                    if ($this->Tsfe->hasTsfe() || $options['bootTsfe']) {
                        $pid = is_null($options['pid']) ? $this->TypoContext->getPidAspect()->getCurrentPid()
                            : $options['pid'];
                        $this->makeDummyTsfe($pid);
                        // Make sure the language aspect stays the same way as we set it...
                        if (isset($languageAspect)) {
                            $this->TypoContext->getRootContext()->setAspect('language', $languageAspect);
                        }
                    }
    ...
    public function runAsAdmin(callable $handler)
    {
        ...
        if ($backupBackendUser instanceof BackendUserAuthentication && is_array($backupBackendUser->user)
            && ! empty($backupBackendUser->user['uid'])) {
        ...
        $this->TypoContext->getRootContext()->setAspect('backend.user',
            $this->getInstanceOf(UserAspect::class, [$this->adminBackendUser]));
        ...
        if (! $language instanceof SiteLanguage) {
            if (! is_null($fallbackLanguage)) {
                if ($fallbackLanguage === true) {
                    $fallbackLanguage = $this->TypoContext->getSiteAspect()->getSite()->getDefaultLanguage();
                }
