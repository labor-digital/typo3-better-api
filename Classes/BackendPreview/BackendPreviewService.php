...
class BackendPreviewService implements SingletonInterface, BackendPreviewServiceInterface, LazyEventSubscriberInterface {
	 ...
	 */
	public function __construct(TypoContainerInterface $container, TranslationService $translationService,
								FlexFormService $flexFormService, TypoContext $context) {
	...
	public function __onBackendContentListLabelRendering(BackendListLabelFilterEvent $event) {
		...
		$fieldSlicer = function ($v) {
			$v = strip_tags($v);
			if (strlen($v) > 100) return trim(substr($v, 0, 100)) . "...";
			return $v;
		};
		foreach ($this->backendListLabelRenderers as $renderer) {
			...
			if (count(array_intersect_assoc($renderer["constraints"], $event->getRow()))
				!== count($renderer["constraints"])) continue;
		...
		if (!$foundLabel) {
			foreach (["headline", "title", "header", "bodytext", "content", "description", "desc"] as $field) {
				if (empty($row[$field]) || !is_string($row[$field]) || is_numeric($row[$field])) continue;
				if (empty(trim(strip_tags($row[$field])))) continue;
	...
	protected function callBackendPreviewRenderer(string $rendererClass, BackendPreviewRenderingEvent $event) {
		...
		if (!$renderer instanceof BackendPreviewRendererInterface)
			throw new BackendPreviewException("The given renderer class: $rendererClass has to implement the correct interface: " . BackendPreviewRendererInterface::class);
		
		// Prepare the row
		$row = $event->getRow();
		$row["settings"] = [];
		if (!empty($row["pi_flexform"])) $row = array_merge($row, $this->flexFormService->convertFlexFormContentToArray($row["pi_flexform"]));
		...
		$this->container->get(EnvironmentSimulator::class)->runWithEnvironment(
			["bootTsfe" => FALSE, "language" => $languageUid], function () use ($renderer, $event, $row) {
			try {
				...
				if (empty($context->getHeader())) $context->setHeader("<b>" . $this->findDefaultHeader($row) . "</b>");
				...
				$event->setAsRendered();
				$event->setContent($this->renderErrorMessage($e->getMessage() . " (" . $e->getFile() . ":" . $e->getLine() . ")"));
			}
		});
	}
	...
	protected function callBackendListLabelRenderer(string $rendererClass, BackendListLabelFilterEvent $event): string {
		try {
			...
			if (!$renderer instanceof BackendListLabelRendererInterface)
				throw new BackendPreviewException("The given renderer class: $rendererClass has to implement the correct interface: " . BackendListLabelRendererInterface::class);
	...
	protected function findDefaultHeader(array $row): string {
		...
		if ($row["CType"] === "list") {
			...
			foreach ($GLOBALS["TCA"]["tt_content"]["columns"]["list_type"]["config"]["items"] as $listTypeItem) {
				if ($listTypeItem[1] !== $signature) continue;
	...
	protected function renderErrorMessage(string $error): string {
		return "<div style=\"background-color:red; padding: 10px; font-family: sans-serif; color: #fff\">" . htmlentities($error) . "</div>";
	}
