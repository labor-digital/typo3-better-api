...
class TypoLink {
	...
	public function __clone() {
		if (!empty($this->request)) $this->request = clone $this->request;
		if (!empty($this->uriBuilder)) $this->uriBuilder = clone $this->uriBuilder;
	}
	...
	public function withRemovedFromFragment(string $key): TypoLink {
		$clone = clone $this;
		if (!is_iterable($clone->fragment)) return $clone;
	...
	public function build(array $options = []): string {
		...
		if ($options["forMe"]) $request = $this->controllerRequest;
		...
		if (empty($request)) $request = $this->context->getRequest();
		...
		// Set config flags
		if (!$options["relative"]) $ub->setCreateAbsoluteUri(TRUE);
		$ub->setUseCacheHash($this->cHash);
		
		// Page id
		if (!empty($this->pid)) $ub->setTargetPageUid($this->pid);
		else $ub->setTargetPageUid($this->context->TypoContext->getPidAspect()->getCurrentPid());
		...
		foreach ($this->args as $k => $v) {
			if (!is_string($v) || substr($v, 1, 4) !== "pid.") continue;
		...
		if (!empty($this->requiredArgs)) {
			...
			foreach ($this->requiredArgs as $arg)
				if (!array_key_exists($arg, $this->args)) $missingArgs[] = $arg;
		...
		if (!empty($this->requiredFragmentArgs)) {
			...
			foreach ($this->requiredFragmentArgs as $arg)
				if (!array_key_exists($arg, (array)$this->fragment)) $missingArgs[] = $arg;
			if (!empty($missingArgs))
		...
		if (isset($backupRequest)) $ub->setRequest($backupRequest);
		if (empty($this->uriBuilder)) $ub->reset();
