...
abstract class AbstractElementConfigurator {
	...
	public function __construct(string $pluginName, ExtConfigContext $context) {
		...
		if (class_exists($controllerClass)) $this->setControllerClass($controllerClass);
	}
	...
	public function setActions(array $actions): AbstractElementConfigurator {
		...
		if (empty($defaultControllerKey)) throw new ExtConfigException("Invalid actions array given!");
	...
	protected function actionsProcessor(array $actions): array {
		...
		if (empty($actions) && class_exists($this->controllerClass)) {
			$actions = [
				$this->pluginName => array_map(function ($v) {
					...
					if ($v === "initializeAction") return FALSE;
		...
		// Filter the actions
		if (Arrays::isSequential($actions)) $actions = [$this->pluginName => $actions];
		foreach ($actions as $k => $list) {
			if (is_string($list)) $list = Arrays::makeFromStringList($list);
			if (!is_array($list)) throw new ExtConfigException("Invalid plugin action array for $this->pluginName given!");
	...
	protected function setControllerClass(string $controllerClass) {
		...
		$controllerBaseName = Inflector::toCamelCase(preg_replace("~controller$~si", "", Path::classBasename($controllerClass)));
		$this->templateRootPaths->push($this->context->TypoContext->getPathAspect()->getTemplatePath($this->context->getExtKey(), $controllerBaseName));
		$this->partialRootPaths->push($this->context->TypoContext->getPathAspect()->getPartialPath($this->context->getExtKey(), $controllerBaseName));
		$this->layoutRootPaths->push($this->context->TypoContext->getPathAspect()->getLayoutPath($this->context->getExtKey(), $controllerBaseName));
