...
abstract class AbstractExtConfigOption implements ExtConfigOptionInterface
{
     ...
     * @param   string|null  $uniqueKey  Can be used if you want to add "unique" values. Unique values will be
     *                                   overwritten if they already exist in the list of values, non unique values
    ...
    protected function runCachedValueGenerator(string $key, $generator, array $additionalData = [])
    {
        ...
        if (is_string($generator) && class_exists($generator)
            && in_array(CachedValueGeneratorInterface::class, class_implements($generator))) {
            $generator = [$generator, 'generate'];
        }
        
        // Check if the generator is a class reference we have to create
        if (is_array($generator) && count($generator) === 2 && is_string($generator[0])
            && class_exists($generator[0])) {
     ...
     * @return $this
     */
    protected function addDirectoryToCachedStack(
        string $stackKey,
        string $directoryPath,
        callable $elementFilter,
        callable $elementKeyProvider,
     ...
     * @return $this
     */
    protected function removeFromCachedStack(
        string $stackKey,
        string $elementKey,
        string $configurationClass,
        bool $overrides = false
    ) {
        return $this->removeFromCachedValueConfig('element' . $stackKey . ($overrides ? '.override' : ''),
            $elementKey . $configurationClass);
    }
     ...
     * @return mixed
     */
    protected function getCachedStackValueOrRun(
        string $listKey,
        string $cachedStackGeneratorClass,
        array $additionalData = []
    ) {
        return $this->getCachedValueOrRun('element' . $listKey,
            function () use ($listKey, $cachedStackGeneratorClass, $additionalData) {
     ...
     * @throws \LaborDigital\Typo3BetterApi\ExtConfig\ExtConfigException
     */
    protected function runCachedStackGenerator(
        string $listKey,
        string $cachedStackGeneratorClass,
        array $additionalData = []
    ) {
    ...
    protected function getChildOptionInstance(string $childOptionClass): AbstractChildExtConfigOption
    {
        ...
        if (! in_array(AbstractChildExtConfigOption::class, class_parents($childOptionClass))) {
            throw new ExtConfigException('Could not create a child for the option: ' . get_called_class() . ' of type: '
                                         .
                                         $childOptionClass . ' because it does not extend the '
                                         . AbstractChildExtConfigOption::class . ' class');
        }
