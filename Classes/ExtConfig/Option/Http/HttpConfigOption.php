...
class HttpConfigOption extends AbstractExtConfigOption
{
    ...
    public function __injectRouteEnhancers(SiteConfigFilterEvent $event)
    {
        ...
        foreach ($routeEnhancers as $k => $config) {
            ...
            if (empty($site)) {
                if ($isMultiSite) {
                    throw new ExtConfigException("Your route enhancer: $k did not define a site key it should be registered for! If you are running in a multi-site setup your route enhancers have to specify a \"site\" option.");
                }
                $site = $defaultSite;
            }
