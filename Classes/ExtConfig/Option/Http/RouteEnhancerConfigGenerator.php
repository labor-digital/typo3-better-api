...
class RouteEnhancerConfigGenerator implements CachedValueGeneratorInterface
{
    ...
    public function generate(array $data, ExtConfigContext $context, array $additionalData, $option)
    {
        ...
        $context->runWithCachedValueDataScope($data,
            function (array $options, string $key) use (&$routeEnhancers, $context, $additionalData) {
                ...
                if (! method_exists($this, $method)) {
                    throw new ExtConfigException('The given route enhancer did specify an enhancer with type: ' .
                                                 $options['routeType']
                                                 . " but there is no configuration method with the name $method in the generator class!");
                }
     ...
     * @return array
     */
    protected function prepareOptionsWithRoutePath(
        array $options,
        ExtConfigContext $context,
        array $additionalDefinition = []
    ): array {
    ...
    protected function prepareRawConfig(array $options): array
    {
        ...
        if (! empty($options['defaults'])) {
            $config['defaults'] = Arrays::merge((is_array($config['defaults']) ? $config['defaults'] : []),
                $options['defaults']);
        }
    ...
    protected function prepareLocaleArgs(array $config, array $options): array
    {
        ...
        foreach ($options['localeArgs'] as $field => $fieldConfig) {
            ...
            $baseError = "Invalid configuration for the localeArgs \"$field\" in the configuration of route: "
                         . $options['routePath'] . '!';
    ...
    protected function prepareStaticArgs(array $config, array $options): array
    {
        ...
        foreach ($options['staticArgs'] as $field => $fieldConfig) {
            // Validate the input
            if (! is_string($field)) {
                throw new ExtConfigException('Invalid configuration for a staticArgs in the configuration of route: '
                                             . $options['routePath'] . '! A numeric key can\'t be an alias name!');
            }
            $baseError = "Invalid configuration for the staticArgs \"$field\" in the configuration of route: "
                         . $options['routePath'] . '!';
            if (! is_array($fieldConfig)) {
                throw new ExtConfigException($baseError . ' The field configuration has to be an array!');
            }
