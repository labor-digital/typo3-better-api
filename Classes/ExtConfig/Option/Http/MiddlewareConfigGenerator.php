...
class MiddlewareConfigGenerator {
	...
	public function generate(array $middlewares, array $disabledMiddlewares): array {
		...
		foreach ($middlewares as $configRaw) {
			...
			if (empty($identifier) || !is_string($identifier)) $identifier = $this->makeMiddlewareIdentifier($config["class"]);
			
			// Prepare ordering
			$before = $after = [];
			if (!empty($config["options"]["before"])) $before = $config["options"]["before"];
			if (is_string($before)) $before = [$before];
			if (!empty($config["options"]["after"])) $after = $config["options"]["after"];
			...
			if (!empty($before)) $c["before"] = $before;
			if (!empty($after)) $c["after"] = $after;
