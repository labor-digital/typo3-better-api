 ...
 *
 * Last modified: 2021.01.14 at 16:50
 */
 ...
 *
 * @see     \LaborDigital\T3BA\Core\DependencyInjection\ContainerAwareTrait
 * @see     \LaborDigital\T3BA\Core\DependencyInjection\StaticContainerAwareTrait
 *
...
class CommonServices implements PublicServiceInterface
{
    ...
    protected $def
        = [
            ...
            'simulator'   => [EnvironmentSimulator::class, true],
    ...
    /**
     * Holds the instance resolver closure
     *
     * @var \Closure
     */
    protected $generator;
    ...
    {
        if (! $generator) {
            $container = TypoContext::getInstance()->di()->getContainer();
            $instances = [];
            $generator = static function (string $class, bool $new = false) use (&$instances, $container) {
                if ($new) {
    ...
    public function __get($name)
    {
        if (! isset($this->def[$name])) {
            // Temporary helper until the object manager is completely removed
            if ($name === 'objectManager') {
        ...
        return ($this->generator)(...$this->def[$name]);
    }
