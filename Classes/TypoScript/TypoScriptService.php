...
class TypoScriptService implements SingletonInterface, LazyEventSubscriberInterface
{
     ...
     *                            addConstant() calls refer to the same title, their content will be merged into a
     *                            single file
     ...
     * @param   array   $options    Additional options
     *                              - extension (string): Descriptive only. Shows beside the title in the backend UI
     *                              - title (string): A unique title for your template. NOTE: If multiple addToSetup()
     *                              or addConstant() calls refer to the same title, their content will be merged into a
     *                              single file
     *
     * @return \LaborDigital\Typo3BetterApi\TypoScript\TypoScriptService
     */
     ...
     * @param   string  $path       default('Configuration/TypoScript/') The default typoScript configuration. But can
     *                              be any path inside the given extension
     * @param   string  $title      An optional title to be displayed in the backend
     *
     * @return \LaborDigital\Typo3BetterApi\TypoScript\TypoScriptService
     */
    public function addStaticTsDirectory(
        string $extension,
     ...
     * @return \LaborDigital\Typo3BetterApi\TypoScript\TypoScriptService
     */
    public function addSelectablePageTsConfigFile(
        string $extension,
        string $path,
        ?string $title = null
    ): TypoScriptService {
     ...
     * @param   bool   $keepTypes  By default the object types are moved into a \@type property of the child. If you
     *                             don't want that set this to false. NOTE: In that case you will loose the types.
     *
     * @return array
     */
    ...
    public function __injectSelectablePageTs(): void
    {
        ...
        if (! empty($this->registeredScripts['selectablePageTs'])) {
            foreach ($this->registeredScripts['selectablePageTs'] as $config) {
                ...
                ExtensionManagementUtility::registerPageTSConfigFile($config['extKey'], $config['path'],
                    $config['title']);
            }
        }
