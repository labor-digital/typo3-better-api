...
class RelationPreset extends AbstractFormPreset {
	...
	public function categorize(array $options = []) {
		...
		if (!empty($options["limitToPids"])) {
			...
			if ($options["limitToPids"] === TRUE) $pidSelector = " = ###CURRENT_PID###";
	...
	public function relationGroup($foreignTable, array $options = []) {
							...
							$this->addMinMaxItemOptions([
								"mmTable"     => [
									...
									"default" => function ($field, $given) {
										...
										if ($given["maxItems"] == 1) return FALSE;
										return TRUE;
									},
								],
								"mmTableName" => [
		...
		if (count($tables) === 1) $config["foreign_table"] = reset($tables);
		...
		if (!empty($filters)) $config["filter"] = $filters;
	...
	public function relationPage(array $options = []) {
		if (!isset($options["maxItems"])) $options["maxItems"] = 1;
		if (!isset($options["allowEdit"])) $options["allowEdit"] = FALSE;
	...
	public function relationFile(array $options = []) {
		...
		if ($this->isInFlexFormSection()) {
			...
			$r = ExtensionManagementUtility::getFileFieldTCAConfig(
				$this->field->getId(), [], $options["allowList"], $options["blockList"]);
		...
		if (!empty($options["baseDir"])) {
			...
			// Allow direct upload
			if (!$this->isInFlexFormSection()) $config["appearance"]["fileUploadAllowed"] = TRUE;
		}
	...
	public function relationImage(array $options = []) {
		...
		if ($options["allowCrop"] && !$this->isFlexForm()) {
			...
			if (!empty($cropVariants)) $cropConfig["cropVariants"] = $cropVariants;
