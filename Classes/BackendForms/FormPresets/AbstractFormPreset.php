...
use LaborDigital\Typo3BetterApi\BackendForms\FlexForms\FlexSection;
...
abstract class AbstractFormPreset implements FormPresetInterface {
	 ...
	 * @inheritDoc
	 */
	public function setContext(ExtConfigContext $context): void {
	...
	protected function addEvalOptions(array $optionDefinition, array $evalFilter = [], array $evalDefaults = []): array {
		foreach (static::EVAL_TYPES as $type)
			if (empty($evalFilter) || in_array($type, $evalFilter))
	...
	protected function addEvalConfig(array $config, array $options, array $evalFilter = []): array {
		...
		$eval = [];
		foreach (static::EVAL_TYPES as $type)
			if ((empty($evalFilter) || in_array($type, $evalFilter)) && $options[$type] === TRUE) $eval[] = $type;
		$evalString = implode(",", $eval);
		
		// Add eval only if we got it configured
		if (!empty($evalString)) $config["eval"] = $evalString;
		else unset($config["eval"]);
		return $config;
	}
	...
	protected function addReadOnlyConfig(array $config, array $options): array {
		if ($options["readOnly"] === TRUE) $config["readOnly"] = TRUE;
		return $config;
	}
	...
	protected function addAllowEditConfig(array $config, array $options): array {
		if (!$options["allowEdit"]) return $config;
	...
	protected function addAllowNewConfig(array $config, array $options): array {
		if (!$options["allowNew"]) return $config;
	...
	protected function addPlaceholderConfig(array $config, array $options): array {
		if (empty($options["placeholder"])) return $config;
	...
	protected function addBasePidConfig(array $config, array $options): array {
		...
		if ($options["limitToBasePid"] && isset($config["basePid"])) $config["limitToBasePid"] = TRUE;
		return $config;
	}
	...
	protected function addMmTableConfig(array $config, array $options): array {
		...
		if ($options["mmTable"] === FALSE) return $config;
		if ($this->isInFlexFormSection()) return $config;
	...
	protected function addMinMaxItemConfig(array $config, array $options): array {
		...
		if ($options["required"] === TRUE) $options["minItems"] = max($options["minItems"], 1);
	...
	protected function generateTableNameList($tableInput): array {
		if (!is_array($tableInput)) {
			if (is_string($tableInput)) $tableInput = Arrays::makeFromStringList($tableInput);
			if (!is_array($tableInput)) throw new BackendFormException("The given value for \$table is invalid! Please use either an array of table names, or a single table as string!");
		}
	...
	protected function getTcaTable(): TcaTable {
		if ($this->isFlexForm()) $form = $this->field->getForm()->getContainingField()->getForm();
		else $form = $this->field->getForm();
		if ($form instanceof TcaTableType) $form = $form->getParent();
		return $form;
	}
