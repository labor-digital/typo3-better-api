...
class FlexForm extends AbstractForm {
	...
	public function getFileName(): ?string {
		if (!$this->context->Fs->hasFile($this->getFlexFormCacheFilePath())) return NULL;
	...
	public function getField(string $id): FlexField {
		...
		$generator = function ($_id = NULL) use ($id) {
			if (!empty($_id)) $id = $_id; // Override external id when path lookup is used...
	...
	public function loadDefinition(string $definition): FlexForm {
		...
		if (substr(strtolower($definition), 0, 5) === "file:") {
			...
			if (!file_exists($filePath)) {
				...
				if (substr($filePath, -4) !== ".xml") $filePath .= ".xml";
		...
		if (!isset($definitionArray["sheets"])) {
			...
			else if (isset($definitionArray["ROOT"]))
				$sheets = ["sDEF" => $definitionArray];
			else throw new BackendFormException("Could not load flex form, as it does not define any \"sheets\".");
		...
		$walker = function (array $list, array $path, $walker) {
			...
			if (empty($path)) {
				...
				foreach ($list as $k => $sheet) {
					...
					if (!empty($label)) $tab->setLabel($label);
					
					// Try to iterate my children
					$children = Arrays::getPath($sheet, ["ROOT", "el"], []);
					if (!empty($children)) $walker($children, [$k], $walker);
				...
				foreach ($list as $k => $element) {
					...
					if (!empty($element["section"])) {
						...
						// Load section labels
						if (!empty($element["title"])) $section->setLabel($element["title"]);
						if (!empty($container["title"])) $section->setContainerItemLabel($container["title"]);
						...
						foreach ($children as $_k => $_element) {
							...
							} else if (is_array($_element["config"]) && (isset($_element["config"]["type"]) || isset($_element["config"]["renderType"]))) {
					...
					if (!empty($element["TCEforms"])) {
						...
						$this->getField($k);
					} else if (is_array($element["config"]) && (isset($element["config"]["type"]) || isset($element["config"]["renderType"]))) {
						// Check if the element is wrongly formatted
		...
		if (!empty($definitionArray["meta"])) $this->meta = $definitionArray["meta"];
			...
			try {
				...
				switch ($type) {
					...
					case self::TYPE_ELEMENT:
						if (!$result instanceof AbstractFormField) throw new Exception();
						return $result;
					case self::TYPE_CONTAINER:
						if (!$this->elIsContainer($result)) throw new Exception();
						return $result;
					case static::TYPE_TAB:
						if (!$this->elIsTab($result)) throw new Exception();
						return $result;
				}
			} catch (Exception $e) {
			}
	...
	protected function parsePath($path): array {
		// Prepare the path
		if (is_string($path)) $path = Arrays::parsePath($path, static::PATH_SEPARATOR);
	...
	protected function getElementByPathInternal($path) {
		...
		foreach ($path as $part) {
			if (!$pointer instanceof AbstractFormContainer) continue;
			$children = $pointer->getChildren();
			if (isset($children[$part])) $pointer = $children[$part];
			else if (isset($children["_" . $part])) $pointer = $children["_" . $part];
			else
	...
	protected function dumpXmlDefinition(): string {
		...
		foreach ($this->getLayoutArray() as $tabId => $tabChildren) {
			...
			if (!empty($tab->getLabel())) $t["TCEforms"]["sheetTitle"] = $tab->getLabel();
			$t["type"] = "array";
			$t["el"] = [];
			...
			foreach ($tabChildren as $k => $child) {
				...
				if (!is_numeric($k) && is_array($child)) {
					...
					if (!empty($section->getLabel())) $sOuter["title"] = $section->getLabel();
					$sOuter["section"] = 1;
					$sOuter["type"] = "array";
