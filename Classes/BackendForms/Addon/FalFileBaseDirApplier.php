...
class FalFileBaseDirApplier implements LazyEventSubscriberInterface {
	...
	public function __onNodeDataFilter(BackendFormNodeFilterEvent $event) {
		...
		if (!isset($data["renderType"]) || $data["renderType"] !== "inline") {
			...
			if (!is_array($c) || !isset($c["type"]) || $c["type"] !== "group" || !isset($c["internal_type"]) || $c["internal_type"] !== "file") return;
			$type = "flex";
		} else {
			// Handle inline elements
			if (!is_array($c) || !isset($c["foreign_table"]) || $c["foreign_table"] !== "sys_file_reference") return;
		}
		
		// Legacy support
		if (isset($c["rootFolder"])) $c["baseDir"] = $c["rootFolder"];
		
		// Ignore if there is no base dir configured
		if (!isset($c["baseDir"])) return;
		
		// Add the directory path in the session storage
		$folders = $this->Session->getBackendSession()->get("dynamicFalFolders", []);
		if ($type === "tca") $folders[$data["tableName"]][$data["fieldName"]] = $c["baseDir"];
		else $folders["flex"]["data" . $data["elementBaseName"]] = $c["baseDir"];
	...
	public function __applyConfiguredFalFolders(array $params) {
		...
		if (empty($request)) {
			...
			if (!empty($params["table"]) && !empty($params["field"])) {
				if (!isset($folders[$params["table"]][$params["field"]])) return $params["uploadFolder"];
			...
			if (!empty($request[0]) && empty($request[4])) {
				// Handle flex form
				if (!isset($folders["flex"][$request[0]])) return $params["uploadFolder"];
				$folderDefinition = $folders["flex"][$request[0]];
			} else {
				// Handle tca field
				if (!isset($request[4])) return $params["uploadFolder"];
				$request = explode("-", $request[4]);
				if (!isset($request[4])) return $params["uploadFolder"];
				$table = $request[2];
				if (!isset($folders[$table])) return $params["uploadFolder"];
				$field = $request[4];
				if (!isset($folders[$table][$field])) return $params["uploadFolder"];
