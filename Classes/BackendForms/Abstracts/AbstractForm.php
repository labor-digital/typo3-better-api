    ...
    public function moveElementInstance(AbstractFormElement $el, string $position): bool
    {
        ...
        if (count($posParts) !== 2) {
            if (count($posParts) === 1) {
                ...
                throw new BackendFormException("Invalid position given, when trying to move element: $el->id (given position: $position)");
            }
        }
        ...
        if ($modifier === 'auto') {
            $modifier = $targetIsContainer || $targetIsTab ? 'bottom' : 'after';
        }
