...
abstract class AbstractFormContainer extends AbstractFormElement {
	...
	public function getElement(string $id) {
		if (isset($this->elements[$id])) return $this->elements[$id];
		if (isset($this->elements["_" . $id])) return $this->elements["_" . $id];
	...
	public function addMultiple(callable $definition) {
		...
		if ($this instanceof TcaTableType) return call_user_func($definition, $this);
	...
	protected function addElement(AbstractFormElement $el, string $position) {
		...
		if (empty($position)) $position = $this->defaultPosition;
	...
	protected function getElementInternal(string $id, int $type = self::TYPE_ALL, array &$finds = NULL) {
		...
		if (isset($elements[$id])) return $elements[$id];
		if (($type === static::TYPE_CONTAINER || $type === static::TYPE_ALL) && isset($containers[$id])) return $containers[$id];
		if (($type === static::TYPE_TAB || $type === static::TYPE_ALL) && isset($tabs[$id])) return $tabs[$id];
	...
	protected function removeElementInternal(string $id, int $type = self::TYPE_ALL): bool {
		try {
			...
			if (isset($this->elements[$id])) $el = $this->elements[$id];
			else if (isset($this->elements["_" . $id])) $el = $this->elements["_" . $id];
			else $el = $this->getElementInternal($id, $type);
			...
			if ($this->elIsContainer($el) || $this->elIsTab($el)) {
				...
				foreach ($el->getChildren() as $child) $el->removeElement($child->getId());
			}
	...
	protected function getSiblingElement(string $id, bool $after = TRUE): AbstractFormElement {
		...
		if (isset($keys[$pos + ($after ? +1 : -1)])) return $this->elements[$keys[$pos + ($after ? +1 : -1)]];
