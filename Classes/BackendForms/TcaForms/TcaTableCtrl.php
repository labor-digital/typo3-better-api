...
class TcaTableCtrl {
	...
	public function getBackendSortColumns(): ?array {
		if (!is_string($this->ctrl["default_sortby"])) return NULL;
		$columns = [];
	...
	public function setLabelAlternativeColumns($columns, bool $force = NULL): TcaTableCtrl {
		...
		if (is_bool($force)) $this->setForceLabelAlternative($force);
		return $this;
	}
	...
	public function getTitle(): string {
		...
		if (empty($title)) return Inflector::toHuman(preg_replace("/^(.*?_domain_model_)/", "", $this->table->getTableName()));
		return $title;
	}
	...
	public function setRaw($key, $value = NULL) {
		if (is_array($key)) $this->ctrl = $key;
		else $this->ctrl[$key] = $value;
		return $this;
	}
	...
	public function getRaw(): array {
		...
		if (isset($ctrl["title"])) $ctrl["title"] = $this->translationService->getTranslationKeyMaybe($ctrl["title"]);
		else $ctrl["title"] = $this->getTitle();
