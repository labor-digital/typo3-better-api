...
class FileInfo {
	...
	public function __construct($file, FalFileService $falFileService, LazyLoading $lazyLoading) {
		...
		if (is_numeric($file) || is_string($file)) $file = $falFileService->getFile($file);
	...
	public function getHash(): string {
		if ($this->isProcessed()) return md5($this->processedFile->getSha1());
	...
	public function getUrl(bool $withHash = TRUE): string {
		$url = NULL;
		if ($this->isProcessed()) $url = $this->processedFile->getPublicUrl();
		else $url = $this->file->getPublicUrl();
	...
	public function getOriginalUrl(bool $withHash = TRUE): string {
		...
		if (!$this->isProcessed()) return $this->getUrl($withHash);
	...
	public function getVideoInfo(): ?VideoFileInfo {
		...
		if (!empty($this->nestedInfo)) return $this->nestedInfo;
