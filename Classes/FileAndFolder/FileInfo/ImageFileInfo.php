...
class ImageFileInfo {
	...
	public function getImageAlignment(): string {
		if (!$this->parent->isFileReference()) return "cc";
		try {
	...
	public function getCropVariants(): array {
		if (!$this->parent->isFileReference()) return [];
		try {
	...
	public function getCroppedUrl(string $type): string {
		if (!$this->parent->isFileReference()) return $this->parent->getUrl();
		$variants = $this->getCropVariants();
		if (!isset($variants[$type])) return $this->parent->getUrl();
