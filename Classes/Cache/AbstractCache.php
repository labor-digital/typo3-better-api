...
abstract class AbstractCache implements SingletonInterface, SimpleTypoCacheInterface {
	...
	public function get($key, $default = NULL) {
		...
		// Validate existence
		if ($this->getTypoCache()->has($key) === FALSE) return $default;
	...
	public function makeCacheKey(...$args): string {
		...
		$hasher = function ($list, $hasher) {
			...
			if (is_iterable($list)) {
				...
				if ($list instanceof LazyLoadingProxy) $list = $list->_loadRealInstance();
				if ($list instanceof AbstractEntity) $list = $list->_getCleanProperties();
				if (method_exists($list, "getUid")) $props[] = get_class($list) . "." . $list->getUid();
				else {
	...
	protected function prepareKey($key): string {
		...
		if (is_string($key) && stripos($key, "sc-") === 0) return $key;
	...
	protected function setInternal($key, $value, $ttl = NULL, $tags = []) {
		...
		if (is_object($value)) {
			if ($value instanceof LazyObjectStorage) $value = $value->toArray();
			if ($value instanceof LazyLoadingProxy) $value = $value->_loadRealInstance();
		}
