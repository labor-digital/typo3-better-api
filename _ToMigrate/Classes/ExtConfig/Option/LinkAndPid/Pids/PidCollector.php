...
class PidCollector
{
     ...
     * @param   bool    $override  By default we will not override pid's set by typoscript. If you want to do so set
     *                             this to true.
     *
    ...
    public function set(string $key, int $pid, bool $override = false): PidCollector
    {
        ...
        if (isset($this->pids[$key]) && ! $override) {
            return $this;
        }
        $this->pids[$key] = $pid;
    ...
    public function setMultiple(array $pids, bool $override = false): PidCollector
    {
        foreach (Arrays::flatten($pids) as $k => $pid) {
            if (! is_string($k)) {
                throw new ExtConfigException('The given key for pid: ' . $pid . ' has to be a string!');
            }
            if (! is_numeric($pid)) {
                throw new ExtConfigException('The given value for pid: ' . $k . ' has to be numeric! Given value: '
                                             . $pid);
            }
